...
if __name__=="__main__":
    f = open("powerlaw_distribution.txt", "w")
    mean = int(sys.argv[1])
    # alpha = 2
    alpha = float(2*mean-1)/(mean-1)
    no_of_elements = 10000
    rnd = [0 for x in range(no_of_elements)]
    rn = randht(no_of_elements, 'powerlaw', alpha)
    for i in range(no_of_elements):
        rnd[i] = int( round(rn[i],0) )
    for item in rnd:
        f.write("%s\n" %item)